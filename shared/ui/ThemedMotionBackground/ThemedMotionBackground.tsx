...
const hexToRgb = (hex: string): [number, number, number] => {
  const regexMatch = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);
  return regexMatch
    ? [
        parseInt(regexMatch[1]!, 16) / 255,
        parseInt(regexMatch[2]!, 16) / 255,
        parseInt(regexMatch[3]!, 16) / 255,
      ]
    : [1, 1, 1];
};
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const updatePlacement = () => {
        ...
        uniforms.rayPos!.value = [0.5 * width * dpr, -0.2 * height * dpr];
      ...
      const loop = (timestamp: number) => {
        ...
        smoothMouseRef.current.x =
          smoothMouseRef.current.x * smoothingFactor +
          mouseRef.current.x * (1 - smoothingFactor);
        smoothMouseRef.current.y =
          smoothMouseRef.current.y * smoothingFactor +
          mouseRef.current.y * (1 - smoothingFactor);
