...
export const RippleGrid: React.FC<Props> = ({
  ...
  const mouseInfluenceShadowRef = useRef(0);
  const mouseInteractionEnabledRef = useRef(mouseInteraction);
...
void main() {
  ...
  gl_FragColor = vec4(color * t * finalFade * opacity, alpha);
    ...
    // === ВАЖНО: начальные значения — чисто константы, НЕ из пропсов ===
    const DEFAULTS = {
      enableRainbow: false,
      gridColor: "#ffffff",
      rippleIntensity: 0.05,
    ...
    const uniforms: Uniforms = {
      ...
      enableRainbow: { value: DEFAULTS.enableRainbow },
      gridColor: { value: hexToRgbLocal(DEFAULTS.gridColor) },
    ...
    const handleMouseMove = (e: MouseEvent) => {
      const el = containerRef.current;
      if (!el || !mouseInteractionEnabledRef.current) return;
    ...
    const handleMouseEnter = () => {
      ...
      mouseInfluenceShadowRef.current = 1.0;
    };

    const handleMouseLeave = () => {
      if (!mouseInteractionEnabledRef.current) return;
      mouseInfluenceShadowRef.current = 0.0;
    };

    window.addEventListener("resize", resize);
    // Вешаем слушатели всегда; внутри проверяем ref-флаг
    ...
    const render = (t: number) => {
      ...
      const targetInfluence = mouseInfluenceShadowRef.current;
    ...
    return () => {
      ...
      if (el && gl.canvas.parentElement === el) el.removeChild(gl.canvas);
    };
  }, []); // зависимостей нет — эффектор «только монтирование»

  // Применяем пропсы к юниформам + обновляем флаг для слушателей
  useEffect(() => {
    ...
    mouseInteractionEnabledRef.current = mouseInteraction;
  }, [
    enableRainbow,
    gridColor,
    rippleIntensity,
